...
def get_dataset(root, img_size = 64):
    ...
    fnames = [f for f in fnames if os.path.splitext(f)[1].lower() in ('.png', '.jpg', '.jpeg', '.bmp')]
    print("len(fnames):", len(fnames), "root:", root)
    compose = [
        T.ToPILImage(),
...
def calc_ssim(img1, img2):
    ...
    if img1_y.ndim == 2:
        ...
        if img1_y.shape[2] == 3:
            ssims = []
            for i in range(3):
                ssims.append(ssim(img1_y[:, :, i], img2_y[:, :, i]))
            return np.array(ssims).mean()
        elif img1_y.shape[2] == 1:
            return ssim(np.squeeze(img1_y), np.squeeze(img2_y))
    else:

...
def mapdataset(generator,datasetloc):
    ...
    for data in tqdm(range(tot_batch)):
        ...
        A = realA.detach().requires_grad_(True)
        fake_B = generator.forward(A)
        grad_output = torch.ones_like(fake_B)
        fake_B.backward(grad_output)
...
def map_saliency(generator):
    ...
    mapdataset(generator, dataset_root)
    # mapdataset(generator, os.path.join(dataset_root, 'DIV2K_valid_HR'))
    # mapdataset(generator, os.path.join(dataset_root, 'Set5'))
    # mapdataset(generator, os.path.join(dataset_root, 'Set14'))
    # mapdataset(generator, os.path.join(dataset_root, 'BSD100'))
    # mapdataset(generator, os.path.join(dataset_root, 'urban100'))
    # mapdataset(generator, os.path.join(dataset_root, 'manga'))
    generator.train()
    return 
...
def test(generator):
    ...
    generator.eval()
    dataset_root = args.data_dir
    testdataset(generator, dataset_root, _logger)
    # testdataset(generator, os.path.join(dataset_root, 'DIV2K_valid_HR'), _logger)
    # testdataset(generator, os.path.join(dataset_root, 'Set5'), _logger)
    # testdataset(generator, os.path.join(dataset_root, 'Set14'), _logger)
    # testdataset(generator, os.path.join(dataset_root, 'BSD100'), _logger)
    # testdataset(generator, os.path.join(dataset_root, 'urban100'), _logger)
    # testdataset(generator, os.path.join(dataset_root, 'manga'), _logger)
    generator.train()
    return 
...
if __name__ == '__main__':
    ...
    netG.load_state_dict(copy.deepcopy(torch.load(str(args.weight)+"/weights/Generator.pth", map_location=device)))   
    # Start Testing
    test(netG)
